...
/datum/planet/vharr/New()
	..()
	planet_vharr = src
	weather_holder = new /datum/weather_holder/vharr(src)

/datum/planet/vharr/update_sun()
	..()
...
/datum/weather_holder/vharr
	temperature = T0C
	allowed_weather_types = list(
		WEATHER_CLEAR		= new /datum/weather/vharr/clear(),
		WEATHER_OVERCAST	= new /datum/weather/vharr/overcast(),
		WEATHER_LIGHT_SNOW	= new /datum/weather/vharr/light_snow(),
		WEATHER_SNOW		= new /datum/weather/vharr/snow(),
		WEATHER_BLIZZARD	= new /datum/weather/vharr/blizzard(),
		WEATHER_RAIN		= new /datum/weather/vharr/rain(),
		WEATHER_STORM		= new /datum/weather/vharr/storm(),
		WEATHER_HAIL		= new /datum/weather/vharr/hail(),
		WEATHER_BLOOD_MOON	= new /datum/weather/vharr/blood_moon(),
		WEATHER_EMBERFALL	= new /datum/weather/vharr/emberfall(),
		WEATHER_ASH_STORM	= new /datum/weather/vharr/ash_storm(),
		WEATHER_FALLOUT		= new /datum/weather/vharr/fallout()
		)
...
/datum/weather/vharr/light_snow
	name = "light snow"
...
/datum/weather/vharr/snow/process_effects()
	..()
...
/datum/weather/vharr/blizzard/process_effects()
	..()
	for(var/turf/simulated/floor/outdoors/snow/S in SSplanets.new_outdoor_turfs) //This didn't make any sense before SSplanets, either
		if(S.z in holder.our_planet.expected_z_levels)
			for(var/dir_checked in GLOB.cardinal)
...
/datum/weather/vharr/rain/process_effects()
	..()
...
/datum/weather/vharr/storm/process_effects()
	..()
	for(var/mob/living/L in living_mob_list)
		if(L.z in holder.our_planet.expected_z_levels)
			var/turf/T = get_turf(L)
...
/datum/weather/vharr/storm/proc/handle_lightning()
...
/datum/weather/vharr/hail/process_effects()
	..()
	for(var/mob/living/carbon/human/H in living_mob_list)
		if(H.z in holder.our_planet.expected_z_levels)
			...
			if(istype(H.get_active_held_item(), /obj/item/melee/umbrella))
				...
				var/obj/item/melee/umbrella/U = H.get_inactive_held_item()
				if(U.open)
					if(show_message)
						to_chat(H, "<span class='notice'>Hail patters gently onto your umbrella.</span>")
					continue
...
/datum/weather/vharr/blood_moon
	name = "blood moon"
	light_modifier = 0.5
	light_color = "#FF0000"
...
/datum/weather/vharr/ash_storm/process_effects()
	..()
	for(var/thing in living_mob_list)
		var/mob/living/L = thing
		if(L.z in holder.our_planet.expected_z_levels)
...
/datum/weather/vharr/fallout/process_effects()
	..()
	for(var/thing in living_mob_list)
		var/mob/living/L = thing
		if(L.z in holder.our_planet.expected_z_levels)
...
/datum/weather/vharr/fallout/proc/irradiate_nearby_turf(mob/living/L)
	if(!istype(L))
		return
